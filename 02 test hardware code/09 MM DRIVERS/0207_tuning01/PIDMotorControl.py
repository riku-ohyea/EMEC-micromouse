...
class DiffDrive:
    def __init__(self, left_motor, right_motor, left_enc, right_enc,
        ...
        self.pid_dist = PID(kp=0.0100, ki=0.0000, kd=0.00015, out_min=-1, out_max=1, i_limit=0.5)
        # Heading PID drives difference encoder to 0 (straight) or to target (turn).
        self.pid_head = PID(kp=0.01, ki=0.0000, kd=0.000050, out_min=-1, out_max=1, i_limit=0.5)

        # PIDMotorControl.py inside DiffDrive.__init__
        self.pid_wall = PID(
             kp=0.008, ki=0.0, kd=0.000,   # start conservative, tune later
             out_min=-0.12, out_max=0.12,
             i_limit=0.4
             )
        ...
        self.done_band_ticks = 5  # stop when within this many ticks
