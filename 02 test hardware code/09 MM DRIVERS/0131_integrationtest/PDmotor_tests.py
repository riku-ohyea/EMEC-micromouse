...
class DiffDrive:
    def __init__(self, left_motor, right_motor, left_enc, right_enc,
        ...
        self.pid_dist = PID(kp=0.0020, ki=0.0000, kd=0.0004, out_min=-0.7, out_max=0.7, i_limit=0.5)
        # Heading PID drives difference encoder to 0 (straight) or to target (turn).
        self.pid_head = PID(kp=0.0030, ki=0.0000, kd=0.0006, out_min=-0.4, out_max=0.4, i_limit=0.5)

        # Motion loop params
        self.dt = 0.02  # 50 Hz
        self.min_power = 0.36  # helps overcome stiction; tune for your robot
    ...
    # ----------------------------
    def estimate_ticks_per_rev(self, seconds=2.0, power=0.3):
        """
...
def make_robot():
    ...
    bot = DiffDrive(
        ...
        wheel_diam_cm=4.5,
        wheelbase_cm=9.6,
        ticks_per_wheel_rev=1400,  # estimated from 7 PPR + quadrature + 50:1
    )
    return bot
