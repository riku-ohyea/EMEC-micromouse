...
try:
    from ucollections import deque   # MicroPython
except ImportError:
    from collections import deque    # CPython (sim)
...
def main_tests():
    ...
    print(tof.get_distance_mm(0)) # left 
    print(tof.get_distance_mm(1)) # centre
    print(tof.get_distance_mm(2)) # right
    
#     bot.move_forward_cm(18)
    bot.rotate_left_deg(90)
...
#     bot.rotate_left_deg(90.3)
    ...
    while not maze.inCenter(mouse.getPosition()):
        ...
        moveOneCellFloodFill(maze, mouse, costs, bot, tof)
...
def moveOneCellFloodFill(maze, mouse, costs, bot, tof):
    ...
    turnAndMove(maze, mouse, current, next_cell, bot, tof)
...
def turnAndMove(maze, mouse, current, next_cell, bot, tof):
    ...
    if Direction.turnLeft(currentDirection) == nextDirection:
        ...
        mouse.turnAround(bot)
        
    # After turning, this is the direction we're about to drive forward in
    facing_now = mouse.getDirection()

    # Bonk ONLY when about to drive forward AND we *know* there's a wall behind
    if shouldBonk(maze, current, facing_now):
#         API.bonk(bot, tof, distance_back_cm=-4.0, recenter_forward_cm=2.0)
        API.bonk(bot, tof, back_power=-0.5, recenter_cm=2.0)

    mouse.moveForward(bot, tof=tof)
...
def shouldBonk(maze, cell, facing_dir):
    back_dir = Direction.turnAround(facing_dir)
    return maze.getWall(cell, back_dir)
