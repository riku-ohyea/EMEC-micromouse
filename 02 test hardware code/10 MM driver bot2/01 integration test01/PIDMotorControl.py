...
class DiffDrive:
    def __init__(self, left_motor, right_motor, left_enc, right_enc,
                 wheel_diam_cm=4.4, wheelbase_cm=9.9,
        ...
        self.pid_dist = PID(kp=0.00475, ki=0.0000, kd=0.000075, out_min=-1, out_max=1, i_limit=0.5)
        # Heading PID drives difference encoder to 0 (straight) or to target (turn).
        self.pid_head = PID(kp=0.00250, ki=0.0000, kd=0.000030, out_min=-1, out_max=1, i_limit=0.5)

        # PIDMotorControl.py inside DiffDrive.__init__
        self.pid_wall = PID(
             kp=0.00510, ki=0.0, kd=0.000,   # start conservative, tune later
             out_min=-1, out_max=1,
             i_limit=0.5
             )

        # how often we try to refresh TOF (ms). Your motor loop is 3ms. TOF cannot keep up.
        self.tof_period_ms = 20


        # Motion loop params
        self.dt = 0.005  #  Hz
        self.min_power = 0.4  # helps overcome stiction; tune for your robot
        self.done_band_ticks = 20  # stop when within this many ticks
        self.max_time_s = 10.0  # safety timeout per command; adjust as needed
    ...
    def drive_raw(self, left_power, right_power, ms):
        """Direct PWM to motors for ms milliseconds (no PID, no encoder target)."""
        self.L.set(left_power)
        self.R.set(right_power)
        utime.sleep_ms(int(ms))
        self.stop()
        
    def drive_until_stall(self, left_power, right_power, max_ms=400, stall_ms=80, min_delta_ticks=2):
        """
        Drive with fixed PWM until encoders stop changing for stall_ms (or until max_ms).
        Useful for 'bonk' into a wall where wheels will stall.
        """
        t0 = utime.ticks_ms()
        last_move_ms = t0

        lastL, lastR = self._get_counts()
        ...
        while True:
            now = utime.ticks_ms()
            if utime.ticks_diff(now, t0) >= max_ms:
                break
            ...
            # if either wheel is still moving, refresh stall timer
            if (dL >= min_delta_ticks) or (dR >= min_delta_ticks):
                last_move_ms = now

            # if neither wheel has moved enough for long enough => stalled
            if utime.ticks_diff(now, last_move_ms) >= stall_ms:
                break
    ...
    def _run_to_targets(self, target_avg_ticks, target_diff_ticks,
                    tof=None, enable_wall_corr=False, wall_corr_cm=7.0):
        """
        ...
        # before while True:
        wall_ok_at_start = False
        if enable_wall_corr and (tof is not None) and (target_diff_ticks == 0):
             # latch the requirement "both side walls from the cell its leaving"
            wall_ok_at_start = tof.has_both_side_walls()

        last_tof_ms = utime.ticks_ms()
        last_wall_err = 0.0
        wall_mode_prev = False
        ...
        while True:
            ...
            # u_head = self.pid_head.update(err_diff, self.dt) # Replacing

            # compute progress in cm from encoder average
            progress_cm = avg / self.ticks_per_cm

            wall_active = (
                (target_diff_ticks == 0) and
                enable_wall_corr and
                wall_ok_at_start and
                (progress_cm < wall_corr_cm) and
                (tof is not None)
            )

#             wall_active = False

            # handle mode transitions (prevents integrator weirdness)
            if wall_active and not wall_mode_prev:
                self.pid_head.reset()   # disable encoder heading correction cleanly
                self.pid_wall.reset()
            if (not wall_active) and wall_mode_prev:
                self.pid_wall.reset()   # leaving wall mode
            wall_mode_prev = wall_active

            u_dist = self.pid_dist.update(err_avg, self.dt)

            if wall_active:
                # refresh TOF at a sane rate and hold last error between updates
                now_ms = utime.ticks_ms()
                if utime.ticks_diff(now_ms, last_tof_ms) >= self.tof_period_ms:
                    e = tof.side_error_mm()  # L_cal - R_cal
                    if e is not None:
                        last_wall_err = e
                    last_tof_ms = now_ms

                # choose sign so it steers the right way:
                # If it steers away from center, flip the sign here.
                u_head = self.pid_wall.update(-last_wall_err, self.dt)

            else:
                # normal encoder heading correction
    ...
    def move_forward_cm(self, cm, tof=None, enable_wall_corr=False, wall_corr_cm=8.5):
        ticks = self.cm_to_ticks(cm)
        self._run_to_targets(
            target_avg_ticks=ticks,
            target_diff_ticks=0,
            tof=tof,
            enable_wall_corr=enable_wall_corr,
            wall_corr_cm=wall_corr_cm
        )
